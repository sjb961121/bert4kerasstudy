...
def evaluate(data):
    ...
    # precision=0
    # recall=0
    f = open('../weights_relation/dev_pred.json', 'w', encoding='utf-8')
    pbar = tqdm()
    for d in data:
...
class Evaluator(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if f1 >= self.best_val_f1:
            self.best_val_f1 = f1
            train_model.save_weights('../weights_relation/best_model.weights')
...
if __name__ == '__main__':
    ...
    train_model.load_weights('../weights_relation/best_model.weights')
    # while True:
    #     sentence = input("请输入句子")
    #     if sentence == "end":
    #         break
    #     spoes = extract_spoes(sentence)
    #     print(spoes)
